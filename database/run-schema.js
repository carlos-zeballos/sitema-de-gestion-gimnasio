...
// Intentar cargar variables de entorno si existen
try {
...
async function run() {
  ...
  // Lista de credenciales comunes para probar la conexión
  ...
  if (!connection) {
    ...
    console.error('Por favor, asegúrate de que MySQL está encendido y verifica las credenciales en tu editor.');
    process.exit(1);
  }

  try {
    console.log('\n⚙️  Ejecutando sentencias del esquema...');
    
    // Ejecutar todo el archivo SQL (aprovechando multipleStatements: true)
    ...
    if (process.env.DB_PASSWORD !== activeConfig.password || process.env.DB_USER !== activeConfig.user) {
      ...
      console.log(`Te sugerimos actualizar tu backend/.env a:`);
      console.log(`DB_USER=${activeConfig.user}`);
      console.log(`DB_PASSWORD=${activeConfig.password}`);
      
      // Intentar actualizar el archivo .env automáticamente
